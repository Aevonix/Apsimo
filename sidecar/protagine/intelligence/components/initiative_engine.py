...
from protagine.initiatives.context_freshness import is_context_fresh
...
class InitiativeConfig:
    ...
    def from_env(cls) -> "InitiativeConfig":
        ...
        return cls(
            contact_neglect_days=_int("PROTAGINE_INITIATIVE_CONTACT_NEGLECT_DAYS", 7),
            goal_block_threshold_days=_int("PROTAGINE_INITIATIVE_GOAL_BLOCK_DAYS", 1),
            health_score_threshold=_float("PROTAGINE_INITIATIVE_HEALTH_THRESHOLD", 70.0),
            calendar_gap_threshold_hours=_float("PROTAGINE_INITIATIVE_GAP_THRESHOLD", 2.0),
            research_task_age_days=_int("PROTAGINE_INITIATIVE_RESEARCH_AGE_DAYS", 1),
            signal_accumulation_threshold=_int("PROTAGINE_INITIATIVE_SIGNAL_THRESHOLD", 10),
            capability_gap_failures=_int("PROTAGINE_CAPABILITY_GAP_FAILURES", 3),
            backup_receipt_path=os.getenv("PROTAGINE_INITIATIVE_BACKUP_RECEIPT") or None,
        )
...
class InitiativeType(str, Enum):
    ...
    # Autonomous work domains (v0.16.0) — Protagine as the agent's
...
_INTRO_HEADROOM = max(0, int(os.environ.get("PROTAGINE_INTRO_HEADROOM", "2")))
...
class InitiativeEngine:
    ...
    Args:
        graph_client: Protagine graph client for relationship/entity data
        event_bus: Protagine event bus for subscribing to relevant events
    ...
    async def _load_blocked_goals(self) -> None:
        ...
        queries = [
            # Protagine schema (status='blocked', blocked_at)
            {
                "query": """
                    MATCH (g:Goal)
    ...
    async def _load_neglected_contacts(self) -> None:
        ...
        # PROTAGINE_HOST_CONTACT_ID default of "owner" never matched anything,
        ...
        try:
            from protagine.intelligence.relationships.signal_floor import (
    ...
    async def _load_subsystem_health(self) -> None:
        """Query graph for degraded Protagine subsystems."""
        if self.graph is None or not hasattr(self.graph, 'driver'):
            return
    ...
    async def _load_capability_gaps(self) -> None:
        ...
        The failure threshold comes from ``PROTAGINE_CAPABILITY_GAP_FAILURES``
    ...
    async def _load_behavioral_patterns(self) -> None:
        ...
        ``protagine/patterns/`` whose pattern_type values
    ...
    async def _generate_follow_ups(self) -> List[Initiative]:
        ...
        from protagine.delivery.reachout_policy import meaningful_reachout_text
    ...
    async def _generate_relationship_suggestions(self) -> List[Initiative]:
        ...
        2. Protagine has a MANAGES edge to the Person
        3. Protagine has at least one communication channel to the Person
        """
        from protagine.identity.resolver import (
            OwnerIdentityError,
            get_identity_resolver,
        ...
        for contact in candidates:
            ...
            # Gate: check if Protagine MANAGES this person
            has_manages = False
    ...
    async def _generate_commitment_initiatives(self) -> List[Initiative]:
        ...
        from Protagine's own commitment store. The OWNER IS A VALID SUBJECT
    ...
    # generators read observations — Protagine never calls external APIs.
    ...
    def _obs_store(self) -> Any:
        ...
        try:
            from protagine.api.routers.observations import get_observation_store
    ...
    async def _generate_capability_gap_initiatives(self) -> List[Initiative]:
        ...
        ``failure_count`` met the PROTAGINE_CAPABILITY_GAP_FAILURES threshold
    ...
    async def _generate_agent_action_initiatives(self) -> List[Initiative]:
        ...
        if last_repo_check and (now - last_repo_check) < timedelta(hours=4):
            ...
            # The working checkout is deployment-specific: PROTAGINE_WORK_REPO
            # names the repo the hygiene check inspects (see action_registry).
            work_repo = os.environ.get("PROTAGINE_WORK_REPO", "").strip()
