...
package=types.ModuleType('protagine_hermes');package.__path__=[sys.argv[3]];sys.modules['protagine_hermes']=package
...
config={'model':{'provider':'custom','default':'fixture-model','base_url':'http://model.fixture/v1'},
 ...
 'toolsets':['protagine_review'],'platform_toolsets':{'cli':['protagine_review']},
 'tools':{'tool_search':{'enabled':False}},
 'plugins':{'enabled':['protagine'],'protagine':{'native_reviews':{'worker':True,'source_home':str(root),
...
with connect(board='default') as db:
 task_id=kb.create_task(db,title='Review measured log volume',body=body,assignee='protagine-reviews',
  created_by='protagine-initiative',tenant='owner',idempotency_key='protagine-initiative:fixture',
...
from protagine_hermes.review_worker import register_worker,ReviewWorker
from hermes_cli.plugins import PluginContext,PluginManifest,get_plugin_manager
manager=get_plugin_manager();register_worker(PluginContext(PluginManifest(name='protagine'),manager),config['plugins']['protagine']['native_reviews'])
from hermes_cli.kanban_db_dispatch import _worker_argv
argv=_worker_argv(task,'protagine-reviews',str(worker))
assert argv[argv.index('--toolsets')+1]=='protagine_review',argv
from model_tools import get_tool_definitions
schemas=get_tool_definitions(enabled_toolsets=['protagine_review'],disabled_toolsets=['kanban'],quiet_mode=True)
names={s['function']['name'] for s in schemas}
assert names=={'protagine_read_work_source','protagine_review_report'},names
...
agent=AIAgent(model='fixture-model',provider='custom',api_key='disposable-fixture',
 ...
 enabled_toolsets=['protagine_review'],disabled_toolsets=['kanban'],max_iterations=2,session_id='review-boundary-fixture')
...
try:
 ...
 result=invoke('protagine_read_work_source',{'source':1})
 ...
 observed=json.loads(invoke('protagine_read_work_source',{'source':1}))
 ...
 for raw in (b'not-json',json.dumps({'schema':'ProtagineRuntimeLoggingV1','path':str(secret)}).encode(),b'x'*16385):
 ...
 result=invoke('protagine_read_work_source',{'source':0,'path':str(secret)})
 ...
 result=invoke('protagine_review_report',{'disposition':'complete','summary':'Observed repeated tool timeout in a bounded current log sample. Propose inspecting its timeout configuration. Historical frequency unknown.','artifacts':[str(secret)]})
 assert 'error' in result,result
 with connect(board='default') as db:assert kb.get_task(db,task_id).status=='running'
 result=invoke('protagine_review_report',{'disposition':'complete','summary':'Artifact: '+str(workspace)+'/'+os.path.relpath(secret,workspace)})
 ...
 result=invoke('protagine_review_report',{'disposition':'complete','summary':'Observed repeated tool timeout in a bounded current log sample. Propose inspecting its timeout configuration. Historical frequency unknown.'})
 with connect(board='default') as db:assert kb.get_task(db,task_id).status=='done',result
 assert canary.read_bytes()==b'canonical-history-canary'
 stale=ReviewWorker(config['plugins']['protagine']['native_reviews']).before_tool(tool_name='protagine_read_work_source')
 assert stale['action']=='block',stale
 # Native absence of the selected plugin grants no fallback toolset.
 manager.unload()
 assert not get_tool_definitions(enabled_toolsets=['protagine_review'],disabled_toolsets=['kanban'],quiet_mode=True,skip_tool_search_assembly=True)
...
def test_native_executor_read_only_review(tmp_path, canonical):
    ...
    native = os.environ.get('AEVA_HERMES_TEST_SOURCE')
    if not python or not native:
        pytest.skip('Requires the selected native Hermes interpreter and source')
    root = Path(__file__).resolve().parents[2]
    home = tmp_path/'hermes'
    env = {key: os.environ[key] for key in ('PATH', 'LANG') if key in os.environ}
    env.update(HOME=str(tmp_path),HERMES_HOME=str(home/'profiles/protagine-reviews'),
    ...
    probe = PROBE
    if canonical:
        probe = probe.replace("'protagine'", "'protagine'").replace("'protagine_review'", "'protagine_review'")
        for name in ('protagine_read_work_source', 'protagine_review_report'):
            probe = probe.replace(name, name.replace('protagine_', 'protagine_'))
